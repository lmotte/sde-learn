...
plot_paths_2d(
    X_tr[:n_plot],
    save_path="../plots/test_sde_identification_2d/true_samples.pdf",
)
...
X_fp = np.zeros((n_fp, n, n_t_fp))
for i in range(n_t_fp):
    X_fp_i = np.random.uniform([x_1_min, x_2_min], [x_1_max, x_2_max], size=(n_fp, n))
    X_fp[:, :, i] = X_fp_i
...
d_p_te_kolmogorov, d_p_te_direct, MSE_te, norms = estimator.compute_mse_te(
    T_grid, X_grid_fp
)
...
def b_pos(t, x):
    b_v = estimator.predict(
        T_te=np.array(t).reshape(1, 1), X_te=np.array(x).reshape(1, n)
    )[0].reshape((2,))
    return b_v
...
paths = euler_maruyama(
    b_pos, sigma_func_pos, n_steps, n_paths_tr, T, n, mean_0, std_0, time=True
)
...
plot_paths_2d(paths, save_path="../plots/test_sde_identification_2d/samples.pdf")

# Compute and plot mean and variance of true SDE w.r.t. time.
mean = np.mean(X_tr, axis=0)
std = np.std(X_tr, axis=0)
save_path = "../plots/test_sde_identification_2d/mean_sample_true.pdf"
simple_plot(
    mean[:, 0],
    mean[:, 1],
    save_path,
...
save_path = "../plots/test_sde_identification_2d/std_sample_true.pdf"
simple_plot(
    std[:, 0],
    std[:, 1],
    save_path,
...
save_path = "../plots/test_sde_identification_2d/mean_sample_es.pdf"
simple_plot(
    mean[:, 0],
    mean[:, 1],
    save_path,
...
save_path = "../plots/test_sde_identification_2d/std_sample_es.pdf"
simple_plot(
    std[:, 0],
    std[:, 1],
    save_path,
